                ...
                <div class="modal-body">
                    ...
                    <div class="box-body table-responsive no-padding">
                        <table class="table table-hover">
                            <th>@lang('backend/_globals.tables.keterangan')</th>
                            <th colspan="3">@lang('backend/_globals.tables.nilai_pengajuan')</th>
                            <th>Total Pengajuan</th>
                            @foreach($jenisbiaya as $key => $jenisbiayas)
                                @if($dataperjadin != null)
                                    @if($jenisbiayas->name == 'Tiket Pesawat')
                                        @php
                                        $nilai2 = number_format(@$dataperjadin->pesawat ,0, ',', '.');
                                        // $nilai3 = $dataperjadin->pj_pesawat;
                                        @endphp
                                    @elseif($jenisbiayas->name == 'Transport / Taksi')
                                        @php
                                        $nilai2 = number_format( @$dataperjadin->taksi_provinsi ,0, ',', '.');
                                        // $nilai3 = $dataperjadin->pj_taksi_provinsi;
                                        @endphp
                                    @elseif($jenisbiayas->name == 'Transport / Taksi (Kab / Kota)')
                                        @php
                                        $nilai2 = number_format(@$dataperjadin->taksi_kab_kota ,0, ',', '.');
                                        // $nilai3 = $dataperjadin->pj_taksi_kab_kota;
                                        @endphp
                                    @elseif($jenisbiayas->name == 'Uang Harian')
                                        @php
                                        $nilai2 = number_format(@$dataperjadin->uang_harian / $dataperjadin->lama ,0, ',', '.');
                                        $nilai3 = number_format(@$dataperjadin->uang_harian ,0, ',', '.');
                                        @endphp
                                    @elseif($jenisbiayas->name == 'Penginapan')
                                        @php
                                        $nilai2 = number_format(@$dataperjadin->penginapan / $dataperjadin->lama,0, ',', '.');
                                        $nilai3 = number_format(@$dataperjadin->penginapan,0, ',', '.');
                                        @endphp
                                    @elseif($jenisbiayas->name == 'Registrasion Fee')
                                        @php
                                        $nilai2 = number_format(@$dataperjadin->registration ,0, ',', '.');
                                        // $nilai3 = $dataperjadin->pj_registration;
                                        @endphp
                                    @endif
                                @endif
                                    ...
                                    @if($jenisbiayas->flag == 1)
                                        <td>
                                            <input class="form-control input-sm" style="text-align: right;"
                                                   id="hari{{ $jenisbiayas->id }}" name="hari[{{ $jenisbiayas->id }}]"
                                                   placeholder="0" type="text" value="@if ($dataperjadin != null){{ $dataperjadin->lama }} @endif">
                                        </td>
                                        ...
                                        <td>
                                            <input class="form-control input-sm" style="text-align: right;"
                                                   ...
                                                   placeholder="0" type="text" value="@if ($dataperjadin != null){{ $nilai2 }} @endif" >
                                        </td>
                                        <td>
                                            <input class="form-control input-sm jumlah" style="text-align: right;"
                                               id="jumlah" name="jumlah[{{ $jenisbiayas->id }}]"
                                               type="text" value="@if ($dataperjadin != null){{ $nilai3 }} @endif" readonly>
                                        </td>
                                    @else
                                        <td colspan="3">
                                            <input class="form-control input-sm" style="text-align: right;"
                                        ...
                                        <td>
                                            <input class="form-control input-sm jumlah" style="text-align: right;"
                                               id="jumlah" name="jumlah[{{ $jenisbiayas->id }}]"
                                               type="text" value="@if ($dataperjadin != null){{ $nilai2 }} @endif" readonly>
                                        </td>
                                    @endif
                                </tr>
                            @endforeach
                            <tr>
                                ...
                                <th><input class="form-control input-sm" style="text-align: right;" id="total2"
                                           value="{{ $dataperjadin != null ? number_format($dataperjadin->pesawat + $dataperjadin->taksi_provinsi + $dataperjadin->taksi_kab_kota + $dataperjadin->uang_harian + $dataperjadin->penginapan + $dataperjadin->registration ,0, ',', '.') : '' }}" name="total2" placeholder="0" type="text" readonly></th>
                            </tr>
                        </table>
                    </div>
                </div>
        ...
        $(document).ready(function(){
            @foreach($jenisbiaya as $key => $jenisbiayas)
                $('#nilai{{ $jenisbiayas->id }}').keyup(function(){
                    ...
                    // if ($('#nilai{{ $jenisbiayas->id }}').val().replace(/\./g, '') < 0) {
