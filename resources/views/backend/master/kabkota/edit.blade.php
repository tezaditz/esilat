...
@foreach($kabkotas as $kabkota)
    <div class="modal fade  " id="edit-modal{{ $kabkota->id }}">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form class="form-horizontal" action="{{ route('master.kabkota.update', $kabkota->id) }}" method="post">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title">@lang('backend/_globals.buttons.edit') @lang('backend/master/kabkota.module')</h4>
                    </div>
                    <div class="modal-body">
                                    ...
                                    <div class="input-group input-group-sm">
                                        <select name="provinsi_id" class="form-control select2" id="provinsi_id" style="width: 100%;">
                                            @foreach($provinsis as $key => $provinsi)
                                                <option value="{{ $provinsi->id }}" {{ $provinsi->id == $kabkota->provinsi_id ? 'selected' : '' }}>{{ $provinsi->title }}</option>
                                            @endforeach
                                        </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9 {{ $errors->has('nama') ? 'has-error' : '' }}">
                                <input type="text" name="nama" class="form-control" id="nama" placeholder="Jakarta" value="{{ $kabkota->nama }}" required>
                                @if($errors->has('nama'))
                                    <span class="help-block">
                                    {{ $errors->first('nama') }}
                                </span>
