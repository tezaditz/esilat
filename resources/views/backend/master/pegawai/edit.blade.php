...
@foreach($pegawais as $pegawai)
    <div class="modal fade" id="edit-modal{{ $pegawai->id }}">
        <div class="modal-dialog">
            <div class="modal-content">
                <form class="form-horizontal" action="{{ route('master.pegawai.update', $pegawai->id) }}" method="post">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title">@lang('backend/_globals.buttons.edit') @lang('backend/master/pegawai.module')</h4>
                    </div>
                            ...
                            @if($jabatans->count() > 0)
                                <select name="jabatan_id" class="form-control select2" id="jabatan_id">
                                    @foreach($jabatans as $key => $jabatan)
                                        <option value="{{ $jabatan->id }}" {{ $jabatan->id == $pegawai->jabatan_id ? 'selected' : '' }}>{{ $jabatan->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            @if($pangkats->count() > 0)
                                <select name="pangkat_id" class="form-control select2" id="pangkat_id" >
                                    @foreach($pangkats as $key => $pangkat)
                                        <option value="{{ $pangkat->id }}" {{ $pangkat->id == $pegawai->pangkat_id ? 'selected' : '' }}>{{ $pangkat->nama }}</option>
                                    @endforeach
                                </select>
                            ...
                            @if($bagians->count() > 0)
                                <select name="bagian_id" class="form-control select2" id="bagian_id">
                                    @foreach($bagians as $key => $bagian)
                                        <option value="{{ $bagian->id }}" {{ $bagian->id == $pegawai->bagian_id ? 'selected' : '' }}>{{ $bagian->nama_bagian }}</option>
                                    @endforeach
                                </select>
