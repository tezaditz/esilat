 ...
 */
?>
@extends('backend.pertanggungjawaban.perjadin.base')

@section('title', trans('backend/pertanggungjawaban.perjadin.index.title', ['total' => $perjadins->total()]), @parent)
            ...
            <div class="box box-success">
                <div class="box-header">
                    ...
                    <div class="box-tools pull-right">
                        ...
                        <form action="{{ route('pertanggungjawaban.perjadin-dalam-negeri.search') }}" method="post" class="pull-right">
                            {{ csrf_field() }}
                        ...
                        <table class="table table-hover">
                            ...
                            @foreach($perjadins as $key => $perjadin)
                                <tr>
                                    ...
                                    <td>{{ $perjadin->provinsi->title  }}</td>
                                    <td class="text-right">{{ number_format($totalpengajuans->where('perjadin_id', $perjadin->id)->where('keterangan', '=', 'perjadin-dalam-negeri')->sum('jumlah_pengajuan') ,0, ',', '.') }}</td>
                                    <td class="text-right">{{ number_format($totalpengajuans->where('perjadin_id', $perjadin->id)->where('keterangan', '=', 'perjadin-dalam-negeri')->sum('pj_jumlah') ,0, ',', '.') }}</td>
                                    ...
                                    <td class="text-right">
                                    @if($perjadin->status->kode_status == 'PR04')
                                        <a href="{{ route('pertanggungjawaban.perjadin-dalam-negeri.pj_detail',[$perjadin->id]) }}" class="btn btn-default btn-flat btn-xs"><i class="fa fa-eye"></i></a>
                                    @else
                                        <a href="{{ route('pertanggungjawaban.perjadin-dalam-negeri.pj_draf',[$perjadin->id]) }}" class="btn btn-default btn-flat btn-xs"><i class="fa fa-eye"></i></a>
                                    @endif
                                    </td>
                                </tr>
                            @endforeach
