            ...
            <div class="modal-content">
                <form class="form-horizontal" action="{{ route('user.update', $user->id) }}" method="post">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">@lang('backend/_globals.buttons.edit') @lang('backend/user.module')</h4>
                    </div>
                                    ...
                                    <div class="input-group input-group-sm">
                                        <select name="bagian_id" class="form-control select2" id="bagian_id" style="width: 100%;">
                                            @foreach($bagians as $key => $bagian)
                                                <option value="{{ $bagian->id }}" {{ $bagian->id == $user->bagian_id ? 'selected' : '' }}>{{ $bagian->nama_bagian }}</option>
                                            @endforeach
                                        </select>
                                        <span class="input-group-btn">
                                            <a href="{{ route('master.bagian.index') }}" class="btn  btn-flat"><i class="fa fa-upload"></i></a>
                                        </span>
                                    </div>
